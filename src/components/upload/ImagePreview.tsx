...
export function ImagePreview({
  ...
  return (
    <div className="flex flex-col items-center gap-4">
      <div className="relative overflow-hidden rounded-2xl border border-zinc-200 bg-zinc-100 aspect-[3/4] w-full max-w-sm">
        {/* eslint-disable-next-line @next/next/no-img-element */}
        <img src={src} alt={alt} className="h-full w-full object-cover" />
      </div>

      {metaLabel && (
        <p className="text-center text-xs font-medium text-zinc-500">{metaLabel}</p>
      )}
